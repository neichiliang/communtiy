...
import life.myheart.community.enums.NotificationStatusEnum;
import life.myheart.community.enums.NotificationTypeEnum;
...
public class CommentService {
    ...
    public void insert(Comment comment, User commentator) {
        String csensitive = sensitiveFilter.filter(comment.getContent());

        if (!StringUtils.isBlank(csensitive)) {
            comment.setStatus(1);
            comment.setStateContent(csensitive);
    ...
    public long updatalike(long id,long count) {
        Comment comment = commentMapper.selectByPrimaryKey(id);
        long comlike = comment.getLikeCount();
        comment.setLikeCount(comlike+count);
        commentMapper.updateByPrimaryKey(comment);
        Comment comment1 = commentMapper.selectByPrimaryKey(id);
        long comlike1 = comment1.getLikeCount();
        return comlike1;
    }
}
